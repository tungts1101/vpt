...
class BarlowTwinTrainer():
    ...
    def feed_one_batch(self, sample):
        self.model.eval()
        with torch.no_grad():
            x1, _ = sample
            x1 = x1.to(self.device, non_blocking=True)
            z1 = self.model(x1)
            logger.info(f"shape of model output: {z1.shape}")

        # normalization layer for the output of the segmentation head
        # self.bn = nn.BatchNorm1d(affine=False)

    def forward_one_batch(self, sample, is_train):
        if is_train:
            self.forward_train_one_batch(sample)
        else:
            self.forward_eval_one_batch(sample)
    
    def forward_eval_one_batch(self, sample):
        """Eval a single (full) epoch on the model using the given
        data loader.
    ...
    def forward_train_one_batch(self, sample):
        """Train a single (full) epoch on the model using the given
        data loader.

        Args:
            sample: tuple of x1 and x2
        Returns:
            loss
        """
        # move data to device
        ...
        if self.cfg.DBG:
            logger.info(f"shape of inputs: {x1.shape, x2.shape}")
        ...
        with torch.set_grad_enabled(True):
            ...
            loss = on_diag + self.cfg.BARLOW.LAMBD * off_diag
    ...
    def train_segmentation(self, train_loader, val_loader, test_loader):
        ...
        for epoch in range(total_epoch):
            # reset averagemeters to measure performance
            losses.reset()
            batch_time.reset()
            data_time.reset()
            ...
            for idx, input_data in enumerate(train_loader):
                ...
                train_loss = self.forward_one_batch(input_data, True)
                break
            break
        
    
    def eval_segmentation(self, train_loader, val_loader, test_loader):
        # TODO
        pass
